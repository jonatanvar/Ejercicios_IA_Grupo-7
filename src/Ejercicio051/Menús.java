...
public class Menús {
    ...
    public static int mostrarMenuPrincipal(Scanner scanner, String idioma) {
        ...
        do {
            System.out.println("\n┌" + String.valueOf(LINEA_H).repeat(ANCHO - 2) + "┐");
            System.out.println(BORDE_IZQ + centrarTexto(Utilidades.obtenerTexto("MENU_TITULO", idioma), ANCHO - 2) + BORDE_DER);
            System.out.println("├" + String.valueOf(LINEA_H).repeat(ANCHO - 2) + "┤");
            System.out.println(BORDE_IZQ + centrarTexto(Utilidades.obtenerTexto("MENU_EJECUTAR", idioma), ANCHO - 2) + BORDE_DER);
            System.out.println(BORDE_IZQ + centrarTexto(Utilidades.obtenerTexto("MENU_SALIR", idioma), ANCHO - 2) + BORDE_DER);
            System.out.println("└" + String.valueOf(LINEA_H).repeat(ANCHO - 2) + "┘");
            System.out.print(Utilidades.obtenerTexto("SELECCIONE", idioma) + ": ");

            if (scanner.hasNextInt()) {
                opcion = scanner.nextInt();
                if (opcion == 1 || opcion == 2) {
                    imprimirLineaConDato(Utilidades.obtenerTexto("SELECCIONE", idioma) + ": " + opcion);
                    return opcion;
                }
            } else {
                scanner.next();
    ...
    public static void mostrarDespedida(String idioma) {
        System.out.println("\n┌" + String.valueOf(LINEA_H).repeat(ANCHO - 2) + "┐");
        System.out.println(BORDE_IZQ + centrarTexto(Utilidades.obtenerTexto("DESPEDIDA", idioma), ANCHO - 2) + BORDE_DER);
        System.out.println("└" + String.valueOf(LINEA_H).repeat(ANCHO - 2) + "┘");
    }
